...
namespace Framework.Core.Types.Specialized
{
    public class Thing : SortedDictionary<string, object>, IThing
    {
        ...
        public object GetProperty(string name)
        {
            ...
            {
                throw new Exception(string.Format("{0}: invalid property name '{1}' in get property", Config.Lib.DEFAULT_ERROR_MSG_PREFIX, name));
            }
        ...
        public void SetProperty(string name, object value)
        {
            if (string.IsNullOrEmpty(name) || !ContainsKey(name))
            {
                throw new Exception(string.Format("{0}: invalid property name '{1}' in set property", Config.Lib.DEFAULT_ERROR_MSG_PREFIX, name));
            }
        ...
        public static IThing Create(params object[] args)
        {
            ...
            {
                throw new Exception(string.Format("{0}: invalid number of parameters '{1}' to item value variant create method", Config.Lib.DEFAULT_ERROR_MSG_PREFIX, args.Length));
            }
            ...
            for (int i = 0; i < args.Length; i = i + 2)
            {
                ...
                {
                    throw new Exception(string.Format("{0}: invalid type for name of property '{1}' to item value variant create method", Config.Lib.DEFAULT_ERROR_MSG_PREFIX, args[i].GetType()));
                }
            }
