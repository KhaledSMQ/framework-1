    ...
    public static class MimeTypeHelper
    {
        ...
        public static string GetMimeTypeFromFilename(string filename, bool silent = false)
        {
            ...
            else
            {
                if (!silent)
                {
                    Throw.WithPrefix(Config.Lib.DEFAULT_ERROR_MSG_PREFIX, "Unable to determine mime type for filename '{0}'", filename);
                }
            }
        ...
        public static string GetFileExtensionFromMimeType(string mimetype, bool silent = false)
        {
            ...
            else
            {
                if (!silent)
                {
                    Throw.WithPrefix(Config.Lib.DEFAULT_ERROR_MSG_PREFIX, "Unable to determine file extension from mime type '{0}'", mimetype);
                }
            }
