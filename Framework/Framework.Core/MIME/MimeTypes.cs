    ...
    public static class MimeTypes
    {
        ...
        public static string GetMimeTypeFromFilename(string filename, bool silent = false)
        {
            ...
            else
            {
                if (!silent)
                {
                    throw new Exception(string.Format("{0} unable to determine mime type for filename '{1}'", Config.DEFAULT_ERROR_MSG_PREFIX, filename));
                }
            }
        ...
        public static string GetFileExtensionFromMimeType(string mimetype, bool silent = false)
        {
            ...
            else
            {
                if (!silent)
                {
                    throw new Exception(string.Format("{0} unable to determine file extension from mime type '{1}'", Config.DEFAULT_ERROR_MSG_PREFIX, mimetype));
                }
            }
