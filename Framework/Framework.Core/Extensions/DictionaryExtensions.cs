    ...
    public static class DictionaryExtensions
    {
        ...
        public static string ApplyTemplate<V>(this IDictionary<string, V> input, string template)
        {
            ...
            Func<string, string> subfunc = (property =>
            {
                ...
                {
                    Throw.WithMessage(Config.DEFAULT_ERROR_MSG_PREFIX, "ApplyTemplate cannot find property with name '{0}'", property);
                }
        ...
        public static void Add<K, V>(this IDictionary<K, V> dict, K key, V value, string message)
        {
            ...
            {
                Throw.WithMessage(Config.DEFAULT_ERROR_MSG_PREFIX, message, key);
            }
        ...
        public static R ParseProperty<R>(this IDictionary<string, string> dict, string name, bool required, R defaultValue, Func<string, R> parseDelegate)
        {
            ...
            if (dict.ContainsKey(name))
            {
                ...
                {
                    Throw.WithMessage(Config.DEFAULT_ERROR_MSG_PREFIX, "Property '{0}' is required, but has an empty value!", name);
                }
            ...
            else
            {
                ...
                if (required)
                {
                    Throw.WithMessage(Config.DEFAULT_ERROR_MSG_PREFIX, "Property '{0}' is required, but was not found!", name);
                }
            }
