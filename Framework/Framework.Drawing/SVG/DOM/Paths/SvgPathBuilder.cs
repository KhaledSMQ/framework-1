...
namespace Framework.Drawing.SVG.DOM.Paths
{
    internal class SvgPathBuilder : TypeConverter
    {
        ...
        /// <summary>
        /// Unparses the specified collection of path segments to a string.
        /// </summary>
        /// <param name="path">A <see cref="object"/> containing path segments.</param>
        public static string UnParse(object path)
        {
            var segments = (SvgPathSegmentList)path;
            string pathSegment = string.Empty;

            for (int i = 0; i < segments.Count; i++)
            {
                string[] segs = segments[i].ToString().Split('.');
                switch (segs[segs.Length - 1])
                {
                    case "SvgMoveToSegment":
                        pathSegment += "M " + (segments[i] as SvgMoveToSegment).End.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgMoveToSegment).End.Y.ToString().Replace(',', '.');
                        break;
                    case "SvgArcSegment":
                        int size = ((segments[i] as SvgArcSegment).Size.ToString().Equals("Small")) ? 0 : 1;
                        int sweep = ((segments[i] as SvgArcSegment).Sweep.ToString().Equals("Positive")) ? 1 : 0;
                        pathSegment += "A " + (segments[i] as SvgArcSegment).RadiusX.ToString().Replace(',', '.') + "," + (segments[i] as SvgArcSegment).RadiusY.ToString().Replace(',', '.') +
                            " " + (segments[i] as SvgArcSegment).Angle.ToString().Replace(',', '.') + " " + size + "," + sweep + " " + ((segments[i] as SvgArcSegment).End.X).ToString().Replace(',', '.') +
                            "," + ((segments[i] as SvgArcSegment).End.Y).ToString().Replace(',', '.');
                        break;
                    case "SvgLineSegment":
                        pathSegment += "L " + (segments[i] as SvgLineSegment).End.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgLineSegment).End.Y.ToString().Replace(',', '.');
                        break;
                    case "SvgQuadraticCurveSegment":
                        pathSegment += "Q " + (segments[i] as SvgQuadraticCurveSegment).ControlPoint.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgQuadraticCurveSegment).ControlPoint.Y.ToString().Replace(',', '.') + " "
                            + (segments[i] as SvgQuadraticCurveSegment).End.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgQuadraticCurveSegment).End.Y.ToString().Replace(',', '.');
                        break;
                    case "SvgCubicCurveSegment":
                        pathSegment += "C " + (segments[i] as SvgCubicCurveSegment).FirstControlPoint.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgCubicCurveSegment).FirstControlPoint.Y.ToString().Replace(',', '.') + " "
                            + (segments[i] as SvgCubicCurveSegment).SecondControlPoint.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgCubicCurveSegment).SecondControlPoint.Y.ToString().Replace(',', '.') + " "
                            + (segments[i] as SvgCubicCurveSegment).End.X.ToString().Replace(',', '.') + "," + (segments[i] as SvgCubicCurveSegment).End.Y.ToString().Replace(',', '.');
                        break;
