...
namespace Framework.Drawing.SVG.DOM
{
    ...
    public abstract class SvgElement : ISvgElement, ISvgTransformable, ICloneable
    {
        ...
        protected virtual void WriteStartElement(XmlTextWriter writer)
        {
            ...
            {
                writer.WriteStartElement(this.ElementName, SvgFragment.Namespace.ToString());
                if (this.ElementName.Contains("svg"))
                    writer.WriteAttributeString("xmlns", "xlink", null, SvgDocument.XLinkNamespace);
        ...
        protected virtual void WriteAttributes(XmlTextWriter writer)
        {
            ...
            foreach (var attr in attributes)
            {
                if (attr.Property.Converter.CanConvertTo(typeof(string)))
                {
                    ...
                    {
                        string value = (string)attr.Property.Converter.ConvertTo(null, System.Globalization.CultureInfo.InvariantCulture, propertyValue, typeof(string));
                        if (!value.Equals(string.Empty) && !value.Equals("none") && !value.Equals("NaN"))
                        {
                            if (attr.Attribute.Name.Equals("href"))
                            {
                                writer.WriteAttributeString("xlink:" + attr.Attribute.Name, value);
                            }
                            else if (attr.Attribute.Name.Equals("visibility"))
                            {
                                if (!Convert.ToBoolean(value))
                                    writer.WriteAttributeString(attr.Attribute.Name, "hidden");
                            }
                            else
                            {
