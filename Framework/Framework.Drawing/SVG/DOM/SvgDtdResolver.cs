...
namespace Framework.Drawing.SVG.DOM
{
    internal class SvgDtdResolver : XmlUrlResolver
    {
        ...
        {
            if (absoluteUri.ToString().IndexOf("toolkit", StringComparison.InvariantCultureIgnoreCase) > -1)
            {
                return Assembly.GetExecutingAssembly().GetManifestResourceStream("Framework.Resources.svg11.dtd");
            }
            else
            {
